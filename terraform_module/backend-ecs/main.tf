...
resource "aws_ecs_task_definition" "ecs-backend-task-def" {
  ...
  memory                   = "512"
  cpu                      = "256"
  execution_role_arn       = var.TASK_EXECUTION_ROLE_ARN # EC2 instance에게 부여되는 role. fargate mode일 경우, container를 직접 실행하므로 "fargate task"를 실행하는 데 사용됨
  # execution role = task 자체 실행 권한
  task_role_arn = var.TASK_EXECUTION_ROLE_ARN # task에서 실행되는 container에게 부여되는 role. e.g.) access s3 bucket
