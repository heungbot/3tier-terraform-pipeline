...
resource "aws_s3_bucket" "main_bucket" {
  bucket = "main-${var.BUCKET_NAME}" # "heungbot-cdn-origin-bucket"
...
# resource "aws_s3_object" "test_main_index" {
#   key                    = "index.html"
#   bucket                 = aws_s3_bucket.main_bucket.id
#   source                 = var.MAIN_INDEX_HTML_PATH
#   server_side_encryption = "aws:kms" # default serverside encryption
# }

##### 02 FAILOVER BUCKET ##### 

resource "aws_s3_bucket" "failover_bucket" {
  bucket = "failover-${var.BUCKET_NAME}"
  tags   = var.FRONTEND_TAG
}

resource "aws_s3_bucket_public_access_block" "failover_bucket_public_access" {
...
# aws_s3_object resource = 한 번에 하나의 object만 upload 가능
# resource "aws_s3_object" "test_failover_index" {
#   key                    = "index.html"
#   bucket                 = aws_s3_bucket.failover_bucket.id
#   source                 = var.FAILOVER_INDEX_HTML_PATH
#   server_side_encryption = "aws:kms" # default serverside encryption
# }
...
resource "aws_cloudfront_distribution" "main_s3_distribution" {
  origin {
    domain_name              = aws_s3_bucket.main_bucket.bucket_regional_domain_name # regional_domain_name = region이 포함된 bucket domain임. cf) bucket_domain_name = [bucketname.s3.amazonaws.com]
    origin_id                = "s3Main"
