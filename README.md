...
프로젝트 소개 : 2달간 이벤트성으로 운영되는 가상의 쇼핑몰 상황을 클라이언트로 설정하여, 이에 따른 요구사항을 정의하고 상황에 맞는 AWS Service(Cloudfront, S3, ALB, ECS, ElastiCache, Aurora)를 사용하였습니다. 또한 이들을 Terraform 모듈로 구성하여 인프라를 코드로 정의하고 Jenkins Pipeline을 통해 Build부터 Deployment까지 자동화 하는 프로젝트 입니다.
...
* 출근하는 직장인을 서비스 고객으로 삼아, 매주 월요일 08:30 - 40분에 "출근길에 물건이라도 건지자!!" 라는 할인 이벤트 진행

* 월요일에 오전에 약 10,000명의 유저가 몰릴 것이라 예상 

* 동일 기간한정 이벤트를 쇼핑몰을 분기마다 서비스 예정
